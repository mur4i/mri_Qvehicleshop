...
$(document).on('click', ".color", function() {
    ...
    $.post("https://mri_Qvehicleshop/changeColor",JSON.stringify({colorR,colorG,colorB}));
});
$(document).on('click', ".category, .category5", function() {
    ...
    $("#title-stock").text("Estoque: " + stock);
    $("#title-price").text("Preço: " + costs.toLocaleString() + "$");
    $("#buy-btn").fadeIn();
    $("#test-btn").fadeIn();
    $("#hide-button").fadeIn();
    ...
    $.post("https://mri_Qvehicleshop/spawnVehicle",JSON.stringify({model}));
});
...
$("#test-accept").click(function() {
    let timer = data.testDrive.testDriveTimer;
    $.post("https://mri_Qvehicleshop/testDrive",JSON.stringify({timer,details}))
})
...
$("#buy").click(function() {
    ...
    $.post("https://mri_Qvehicleshop/buyVehicle",JSON.stringify({details}));
})
    ...
    if (key == "Escape"){
        if(!onBuyPage){
            if(vehiclesCategory){
                ...
                $("#hide-button").text("Pré-visualizar");
                ...
                $.post("https://mri_Qvehicleshop/deletevehicle",JSON.stringify({}));
            }else{
                $("#vehicle-class-title-container").fadeOut();
                $("#vehicle-class-container").fadeOut();
                $.post("https://mri_Qvehicleshop/closeVehicleShop",JSON.stringify({}));
            }
        }
...
$("#hide-button").click(function(){
    if(!hideDisplay){
        ...
        $("#vehicle-class-title-container").fadeOut();
        $("#btn-container").fadeOut();
        $("#hide-button").text("Voltar");
        setTimeout(() => {
            $("#hide-button").css("margin-top","450px");
        }, 200);
        $.post("https://mri_Qvehicleshop/zoomCam",JSON.stringify({}));
    } else{
        hideDisplay = false;
        inspect = false;
        $("#hide-button").css("margin-top","2px");
        $.post("https://mri_Qvehicleshop/returnCam",JSON.stringify({}));
        setTimeout(() => {
            $("#vehicle-container").fadeIn();
            $("#title-container").fadeIn();
            $("#btn-container").fadeIn();
...
document.onmousedown = function(e){
    if(inspect){
        if (vehicleDisplay && !down && e.button == 0){
            down = true;
            $.post("https://mri_Qvehicleshop/mousedown",JSON.stringify({}));
        }
    }
}
...
document.onmouseup = function(e){
    if(inspect){
        if (down && e.button == 0){
            $.post("https://mri_Qvehicleshop/mouseup",JSON.stringify({}))
            down = false;
        }
    }
}
...
window.onmousewheel = function(e){
    if(inspect){
        ...
        $.post("https://mri_Qvehicleshop/" + (st < 0 ? "downscroll" : "upscroll"),JSON.stringify({}));
    }
}
...
function changeTime(duration) {
    ...
    $("#test-time").text(minutos + ":" + segundos)
